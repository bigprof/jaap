<?php
	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(   
			'trans_mrs2016' => array('Verkoop badeendrace 2016', '', 'resources/table_icons/anchor.png', 'Badeendrace Maarssen'),
			'duck_mrs2016' => array('Badeenden 2016', '', 'resources/table_icons/anchor.png', 'Badeendrace Maarssen')
		);
	...
	function get_table_groups($skip_authentication = false){
		...
		$all_groups = array('Badeendrace Maarssen');
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'trans_mrs2016' => "`trans_mrs2016`.`transaction_id` as 'transaction_id', `trans_mrs2016`.`firstname` as 'firstname', `trans_mrs2016`.`lastname` as 'lastname', `trans_mrs2016`.`email` as 'email', `trans_mrs2016`.`phone` as 'phone', `trans_mrs2016`.`quantity` as 'quantity', `trans_mrs2016`.`amount` as 'amount', `trans_mrs2016`.`mailinglist` as 'mailinglist', `trans_mrs2016`.`remarks` as 'remarks', if(`trans_mrs2016`.`transactiondate`,date_format(`trans_mrs2016`.`transactiondate`,'%d-%m-%Y'),'') as 'transactiondate', `trans_mrs2016`.`seller` as 'seller', if(`trans_mrs2016`.`editingdate`,date_format(`trans_mrs2016`.`editingdate`,'%d-%m-%Y'),'') as 'editingdate', `trans_mrs2016`.`editor` as 'editor'",
			'duck_mrs2016' => "`duck_mrs2016`.`duck_id` as 'duck_id', IF(    CHAR_LENGTH(`trans_mrs20161`.`transaction_id`), CONCAT_WS('',   `trans_mrs20161`.`transaction_id`), '') as 'transaction_id', `duck_mrs2016`.`creationdate` as 'creationdate'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'trans_mrs2016' => "`trans_mrs2016` ",
			'duck_mrs2016' => "`duck_mrs2016` LEFT JOIN `trans_mrs2016` as trans_mrs20161 ON `trans_mrs20161`.`transaction_id`=`duck_mrs2016`.`transaction_id` "
		);

		$pkey = array(   
			'trans_mrs2016' => 'transaction_id',
			'duck_mrs2016' => 'duck_id'
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('Badeend_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('Badeend_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['Badeend_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['Badeend_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Badeend</a>
			</div>
